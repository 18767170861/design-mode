 ...
 *   而把影响对象行为的一个或多个动态变化的属性称为状态。当有状态的对象与外部事件产生互动时，其内部状态就会发生改变，从而使其行为也发生改变。
 *   如人都有高兴和伤心的时候，不同的情绪有不同的行为，当然外界也会影响其情绪变化。
 *
 *   对这种有状态的对象编程，传统的解决方案是：将这些所有可能发生的情况全都考虑到，然后使用 if-else 或 switch-case 语句来做状态判断，再进行不同情况的处理。
 *   但是显然这种做法对复杂的状态判断存在天然弊端，条件判断语句会过于臃肿，可读性差，且不具备扩展性，维护难度也大。
 *   且增加新的状态时要添加新的 if-else 语句，这违背了“开闭原则”，不利于程序的扩展。
 *
 *   以上问题如果采用“状态模式”就能很好地得到解决。
 *   状态模式的解决思想是：当控制一个对象状态转换的条件表达式过于复杂时，把相关“判断逻辑”提取出来，用各个不同的类进行表示，
 ...
 *
 *   其结构图如图 state.jpg 所示。
 *   2. 模式的实现 {@link org.wmc.behavior.state.StatePatternClientMain}
 *
 * 状态模式的应用实例:
 *   用“状态模式”设计一个学生成绩的状态转换程序 {@link org.wmc.behavior.state.score.ScoreStateMain}
 *
 *   用“状态模式”设计一个多线程的状态转换程序: {@link org.wmc.behavior.state.thread.ScoreStateMain}
 *
 * 状态模式的应用场景:
 *   通常在以下情况下可以考虑使用状态模式。
 ...
 *
 * 状态模式的扩展: {@link org.wmc.behavior.state.extend.FlyweightStatePatternMain}
 *   在有些情况下，可能有多个环境对象需要共享一组状态，这时需要引入享元模式，将这些具体状态对象放在集合中供程序共享，其结构图如图 state_extend.jpg 所示。
