 ...
 *
 *   中介者模式的结构图如图 mediator.jpg 所示。
 *
 *   2. 模式的实现 {@link org.wmc.behavior.mediator.MediatorPatternMain}
 *
 * 模式的应用实例:
 *   用中介者模式编写一个“韶关房地产交流平台”程序 {@link org.wmc.behavior.mediator.estate.DatingPlatformMain}
 *
 * 模式的应用场景:
 ...
 *
 * 模式的扩展: {@link org.wmc.behavior.mediator.extend.SimpleMediatorPatternMain}
 *   在实际开发中，通常采用以下两种方法来简化中介者模式，使开发变得更简单。
 *     不定义中介者接口，把具体中介者对象实现成为单例。
 *     同事对象不持有中介者，而是在需要的时候直接获取中介者对象并调用。
 *
 *   图 mediator_extend.jpg 所示是简化中介者模式的结构图。
 *
 *
 *
 *
