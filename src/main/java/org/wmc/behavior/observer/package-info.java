 ...
 *
 *   观察者模式的结构图如图 observer.jpg 所示。
 *
 *   2. 模式的实现 {@link org.wmc.behavior.observer.ObserverPatternMain}
 *
 * 模式的应用实例:
 *   利用观察者模式设计一个程序，分析“人民币汇率”的升值或贬值对进口公司进口产品成本或出口公司的出口产品收入以及公司利润率的影响 {@link org.wmc.behavior.observer.rate.RMBrateMain}
 *
 *   观察者模式在软件幵发中用得最多的是窗体程序设计中的事件处理，窗体中的所有组件都是“事件源”，也就是目标对象，而事件处理程序类的对象是具体观察者对象。
 *   下面以一个学校铃声的事件处理程序为例，介绍 Windows 中的“事件处理模型”的工作原理。
 *
 *   利用观察者模式设计一个学校铃声的事件处理程序 {@link org.wmc.behavior.observer.ring.BellEventMain}
 *
 * 模式的应用场景:
 ...
 *     多形层级嵌套使用，成一种链式触发机制，使得事件具备跨域（跨越两种观察者类型）通知。
 *
 * 模式的扩展:
 ...
 *
 *   【例3】利用 Observable 类和 Observer 接口实现原油期货的观察者模式实例。 {@link org.wmc.behavior.observer.oil.CrudeOilFuturesMain}
 *
 *     分析：当原油价格上涨时，空方伤心，多方高兴；当油价下跌时，空方局兴，多方伤心。
