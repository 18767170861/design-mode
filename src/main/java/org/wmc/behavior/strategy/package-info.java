 ...
 *
 * 其结构图如图 strategy.jpgCollections 所示。
 *
 * 策略模式的应用实例:
 *   策略模式在“大闸蟹”做菜中的应用 {@link org.wmc.behavior.strategy.cook.CrabCookingStrategyMain}
 *   用策略模式实现从韶关去婺源旅游的出行方式
 *     分析：从韶关去婺源旅游有以下几种出行方式：坐火车、坐汽车和自驾车，所以该实例用策略模式比较适合，图 trip.jpg 所示是其结构图。
 *
 * 策略模式的应用场景:
