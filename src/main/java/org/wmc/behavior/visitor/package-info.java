 ...
 *
 *   其结构图如图 visitor.jpg 所示。
 *
 *   2. 模式的实现 {@link org.wmc.behavior.visitor.VisitorPatternMain}
 *
 * 模式的应用实例:
 *   利用“访问者（Visitor）模式”模拟艺术公司与造币公司的功能 {@link org.wmc.behavior.visitor.company.VisitorProducerMain}
 *
 * 模式的应用场景:
 ...
 *     如【例1】中的对象结构是用 List 实现的，它通过 List 对象的 Iterator() 方法获取迭代器。
 *     如果对象结构中的聚合类没有提供迭代器，也可以用迭代器模式自定义一个。
 *
 *     (2)访问者（Visitor）模式同“组合模式”联用。因为访问者（Visitor）模式中的“元素对象”可能是叶子对象或者是容器对象，
 *     如果元素对象包含容器对象，就必须用到组合模式，其结构图如图 visitor_extend.jpg 所示。
 *
 *
 *
 *
