 ...
 *   3) 语法树
 *     语法树是句子结构的一种树型表示，它代表了句子的推导结果，它有利于理解句子语法结构的层次。图 1.jpg 所示是“我是大学生”的语法树。
 *
 ...
 *
 * 解释器模式的结构图如图 2.jpg 所示。
 *
 * 2. 模式的实现
 *   解释器模式实现的关键是定义文法规则、设计终结符类与非终结符类、画出结构图，必要时构建语法树，
 *
 * 模式的应用实例:
 *   用解释器模式设计一个“韶粵通”公交车卡的读卡器程序 {@link org.wmc.behavior.interpreter.cardreader.InterpreterPatternMain}
 *
 * 模式的应用场景:
 ...
 *   Java 提供了以下强大的数学公式解析器：Expression4J、MESP(Math Expression String Parser) 和 Jep 等，它们可以解释一些复杂的文法，功能强大，使用简单。
 *
 *   现在以 Jep 为例来介绍该工具包的使用方法。Jep 是 Java expression parser 的简称，即 Java 表达式分析器，它是一个用来转换和计算数学表达式的 Java 库。
 ...
 *   在 Eclipse 的“Java 构建路径”对话框的“库”选项卡中选择“添加外部 JAR(X)...”，将该 Jep 包添加项目中后即可使用其中的类库。
 *
 *   下面以计算存款利息为例来介绍。存款利息的计算公式是：本金x利率x时间=利息，其相关代码：{@link org.wmc.behavior.interpreter.jep.JepMain}
 *
 *
 *
 *
