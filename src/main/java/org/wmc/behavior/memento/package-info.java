 ...
 *
 *   备忘录模式的结构图如图 memento.jpg 所示。
 *
 *   2. 模式的实现 {@link org.wmc.behavior.memento.MementoPatternMain}
 *
 * 模式的应用实例:
 *   利用备忘录模式设计相亲游戏 {@link org.wmc.behavior.memento.game.photo.DatingGameMain}
 *
 * 模式的应用场景:
 ...
 *
 * 模式的扩展:
 *   在前面介绍的备忘录模式中，有单状态备份的例子，也有多状态备份的例子。{@link org.wmc.behavior.memento.extend.PrototypeMementoMain}
 *   下面介绍备忘录模式如何同原型模式混合使用。
 *     在备忘录模式中，通过定义“备忘录”来备份“发起人”的信息，而原型模式的 clone() 方法具有自备份功能，
 *     所以，如果让发起人实现 Cloneable 接口就有备份自己的功能，这时可以删除备忘录类，其结构图如图 memento_extend.jpg 所示。
 *
 * 拓展:
