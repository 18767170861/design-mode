 ...
 *
 *   其结构图如图 bridge.jpg 所示。
 *
 *   2. 模式的实现 {@link org.wmc.structure.bridge.BridgeMain}
 *
 * 桥接模式的应用实例:
 *   用桥接（Bridge）模式模拟女士皮包的选购 {@link org.wmc.structure.bridge.bag.BagManage}
 *
 * 桥接模式的应用场景:
 ...
 *   当桥接（Bridge）模式的实现化角色的接口与现有类的接口不一致时，可以在二者中间定义一个适配器将二者连接起来，其具体结构图如图 bridgeExtend.jpg 所示。
 *
 *
 *
 *
