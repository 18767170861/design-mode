 ...
 *    在计算机系统中，还有 Windows 的回收站、操作系统中的文件系统、多线程中的线程池、显卡的驱动程序对象、打印机的后台处理服务、应用程序的日志对象、数据库的连接池、网站的计数器、
 * Web 应用的配置对象、应用程序中的对话框、系统中的缓存等常常被设计成单例。
 ...
 *    访问类：使用单例的类。
 *    其结构如图 singleton.jpg 所示。
 * 2. 单例模式的实现
 *    Singleton 模式通常有两种实现形式。
 *    第 1 种：懒汉式单例 {@link org.wmc.create.singleton.LazySingleton}
 *    第 2 种：饿汉式单例 {@link org.wmc.create.singleton.HungrySingleton}
 *
 * 单例模式的应用实例:
 *   用饿汉式单例模式模拟产生猪八戒对象。{@link }
 *
 * 单例模式的扩展:
 *    单例模式可扩展为有限的多例（Multitcm）模式，这种模式可生成有限个实例并保存在 ArrayList 中，客户需要时可随机获取
 *    其结构图如图 singleton.jpg 所示。
 */
